...
typedef struct MpmTableElmt_ {
    ...
    uint32_t (*Search)(struct MpmCtx_ *, struct MpmThreadCtx_ *, PatternMatcherQueue *, const uint8_t *, uint16_t);
 ...
 */
static inline void
MpmAddSids(PatternMatcherQueue *pmq, sig_id *sids, uint32_t sids_size)
{
    if (sids_size == 0)
        return;
